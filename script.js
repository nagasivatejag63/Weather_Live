 function dateFormat(timestamp) {
        ...
        async function fetchAQIData(lat, lon) {
            let fetchAQIData = await fetch(`https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=9a788bdecf88c32f1670b710a28182ae&units=metric`);
        ...
        async function nextSixDays(lat, lon) {

            const apiUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=9a788bdecf88c32f1670b710a28182ae&units=metric`;
            try {
                ...
                // Get first 6 unique days
                let forecastHtml = "";
                Object.keys(dailyForecasts).slice(0, 6).forEach(date => {
                    let forecast = dailyForecasts[date];
                    forecastHtml += `
                        <div class="forecastRow d-flex align-items-center justify-content-between">
                            <div class="d-flex gap-1 align-items-center">
                                <img src="./Images/cloud.png" alt="server error" width="35px">
                ...
                selectedHours.forEach(item => {
                    ...
                    todayHtml += `
                        <div class="todayTemp">
                            ...
                            <img src="./Images/cloudy.png" alt="server error" width="35px">
                            <h5>${temp}&deg;C</h5>
                        </div>
                    `;
                });
        ...
        async function fetchData() {
            ...
            let requestData = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=9a788bdecf88c32f1670b710a28182ae&units=metric`);
